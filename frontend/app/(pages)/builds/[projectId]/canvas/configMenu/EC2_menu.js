...
export default function EC2PanelForm({ id, onClose, onDelete, label }) {
  ...
  const [saved, setSaved] = useState(
    () => typeof window !== "undefined" && localStorage.getItem(savedKey) === "1"
  );
  ...
  const schema = z.object({
    name: z
      ...
      .min(1, "Required")
      .max(255)
      .regex(nameRegex, "Only letters, numbers, hyphens, and underscores."),
    instanceType: z.string().min(1, "Select an instance type"),
    imageId: z.enum(["Ubuntu", "Amazon Linux", "Windows"], {
      required_error: "Select an image",
    }),
  ...
  useEffect(() => {
    const foundry = async () => {
      try {
        const endpoint = await axios.get(`http://127.0.0.1:8000/canvas/endpoint/`, {
  ...
  const submit = (values) => {
    ...
    const sendWebhookRequest = async () => {
      try {
        await axios.post(
          "https://overslack-stonily-allegra.ngrok-free.dev/github/add_webhook",
          {
            owner: owner,
            repo: repoName,
          }
        ...
        <div className="flex-1 overflow-y-auto space-y-3 p-3 text-xs">
          <div className="rounded-lg bg-gray-50 p-2">
            ...
            <p className="mt-0.5 text-gray-500">
              Configure your EC2 instance with required parameters.
            </p>
          </div>
            ...
            <div>
              {endpoint && (
                <label className="font-medium text-gray-800">
                  endpoint:{" "}
                  <a className="text-blue-700" href={endpoint} target="__blank">
                    {endpoint}
                  </a>
                </label>
              )}
              ...
              render={({ field }) => (
                <Select value={field.value} onValueChange={field.onChange}>
                  ...
                  <SelectContent className="max-h-28 overflow-y-auto bg-gray-200 rounded-lg shadow-lg">
                    {repos?.map((repo) => (
                      <SelectItem key={repo.id} value={`${repo.name}/${repo.owner}`}>
                        {repo.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
