...
import { X, CheckCircle, AlertCircle, Loader2, Copy, Clock, Eye, EyeOff, Server, Database, HardDrive, Table } from 'lucide-react';

// Resource status to UI mapping
const STATUS_CONFIG = {
  'CREATE_IN_PROGRESS': { icon: Loader2, color: 'text-blue-600', bgColor: 'bg-blue-50', borderColor: 'border-blue-200', label: 'Creating', spin: true },
  'CREATE_COMPLETE': { icon: CheckCircle, color: 'text-green-600', bgColor: 'bg-green-50', borderColor: 'border-green-200', label: 'Complete', spin: false },
  'CREATE_FAILED': { icon: AlertCircle, color: 'text-red-600', bgColor: 'bg-red-50', borderColor: 'border-red-200', label: 'Failed', spin: false },
  'UPDATE_IN_PROGRESS': { icon: Loader2, color: 'text-blue-600', bgColor: 'bg-blue-50', borderColor: 'border-blue-200', label: 'Updating', spin: true },
  'UPDATE_COMPLETE': { icon: CheckCircle, color: 'text-green-600', bgColor: 'bg-green-50', borderColor: 'border-green-200', label: 'Updated', spin: false },
  'UPDATE_FAILED': { icon: AlertCircle, color: 'text-red-600', bgColor: 'bg-red-50', borderColor: 'border-red-200', label: 'Update Failed', spin: false },
  'ROLLBACK_IN_PROGRESS': { icon: Loader2, color: 'text-orange-600', bgColor: 'bg-orange-50', borderColor: 'border-orange-200', label: 'Rolling Back', spin: true },
  'ROLLBACK_COMPLETE': { icon: AlertCircle, color: 'text-orange-600', bgColor: 'bg-orange-50', borderColor: 'border-orange-200', label: 'Rolled Back', spin: false },
};
...
function DeploymentModal({ isOpen, onClose, stackName, keyPairs }) {
  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      try {
        const ws = new WebSocket(`ws://localhost:8000/canvas/deploy/track/${stackName}`);
        wsRef.current = ws;
  ...
  const handleResourceUpdate = (data) => {
    ...
    setResources(prev => {
      const existing = prev.findIndex(r => r.logicalId === resource.logicalId);
      const newResource = {
        ...resource,
  ...
  const handleStackComplete = (data) => {
    ...
    if (data.stack.status === 'CREATE_FAILED' || data.stack.status === 'UPDATE_FAILED') {
      setIsFailed(true);
    }
  };
  ...
  const statusColor = isFailed ? 'bg-red-600' : isComplete ? 'bg-green-600' : 'bg-blue-600';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full h-full max-w-7xl max-h-[90vh] flex flex-col animate-in fade-in zoom-in duration-200">
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex-shrink-0">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600 font-mono mt-1">{stackName}</p>
            </div>
        ...
        <div ref={containerRef} className="flex-1 flex overflow-hidden relative">
          {/* Left Side - Logs */}
          <div className="border-r border-gray-200 flex flex-col" style={{ width: `${dividerPosition}%` }}>
            <div className="px-6 py-3 border-b border-gray-200 bg-gray-50">
              <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wide">Activity Logs</h3>
            </div>
            <div
              ref={logsRef}
            ...
            >
              {logs.length === 0 ? (
                ...
                <div className="space-y-1 font-mono text-xs">
                  {logs.map((log) => {
                    const statusConfig = STATUS_CONFIG[log.status] || STATUS_CONFIG['CREATE_IN_PROGRESS'];
                    return (
                      <div key={log.id} className="flex gap-3 text-gray-700">
                        <span className="text-gray-500 flex-shrink-0">{log.timestamp}</span>
                        <span className={`${statusConfig.color} flex-shrink-0`}>
                          {log.status.includes('COMPLETE') ? '[SUCCESS]' : 
                           log.status.includes('FAILED') ? '[ERROR]' : 
                           log.status.includes('PROGRESS') ? '[INFO]' : '[UPDATE]'}
                        </span>
              ...
              {resources.length === 0 ? (
                ...
                <div className="divide-y divide-gray-200">
                  {resources.map((resource) => {
                    const statusConfig = STATUS_CONFIG[resource.status] || STATUS_CONFIG['CREATE_IN_PROGRESS'];
                    const StatusIcon = statusConfig.icon;
                    const ServiceIcon = getServiceIcon(resource.type);
                    const hasError = resource.statusReason && resource.statusReason.length > 0;
                    const isResourceComplete = resource.status.includes('COMPLETE') && !resource.status.includes('ROLLBACK');

                    // Find outputs related to this resource
                    const resourceOutputs = stackOutputs.filter(output => 
                      output.key.toLowerCase().includes(resource.logicalId.toLowerCase())
                    );
                    ...
                    return (
                      <div key={resource.logicalId} className="px-6 py-3 hover:bg-gray-50 transition-colors">
                        {/* Resource Header */}
                        <div className="flex items-center justify-between gap-3">
                          <div className="flex items-center gap-3 flex-1 min-w-0">
                            <ServiceIcon className="w-4 h-4 text-gray-600 flex-shrink-0" />
                            <div className="flex-1 min-w-0">
                              <h4 className="text-sm font-semibold text-gray-900 truncate">{resource.logicalId}</h4>
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            ...
                            {isResourceComplete && keyPairForResource && (
                              <button
                                onClick={() => downloadKeyPair(keyPairForResource.keyName, keyPairForResource.keyMaterial, keyPairForResource.instanceName)}
                            ...
                            {isEC2 ? (
                              ...
                              <>
                                ...
                                {resourceOutputs.map((output, idx) => {
                                  const isPassword = output.key.toLowerCase().includes('password') || 
                                                     output.key.toLowerCase().includes('secret');
                                  const isVisible = visiblePasswords.has(output.key);

                                  return (
                                    <div key={idx} className="flex items-center gap-2">
                                      <span className="text-gray-600 font-medium w-24 flex-shrink-0 truncate" title={output.description || output.key}>
                                        {output.description || output.key}:
                                      </span>
                                      <code className="flex-1 text-gray-900 font-mono bg-gray-100 px-2 py-1 rounded truncate">
                                        {isPassword && !isVisible ? '••••••••••••' : output.value}
                                      </code>
                                      {isPassword && (
                                        <button
                                          onClick={() => togglePasswordVisibility(output.key)}
                                          className="p-1 text-gray-500 hover:text-gray-900 transition-colors"
            ...
            <div>
              <div className="flex items-center justify-between text-sm text-gray-700 mb-2">
                <span className="font-medium">
                  {stackInfo ? `${stackInfo.completedResources} of ${stackInfo.totalResources} resources` : 'Initializing...'}
                </span>
                <span className="font-semibold">{isNaN(progress) ? 0 : progress}%</span>
              </div>
