...
export function CostMonitoringPage({
  onBackToHome,
  services,
}: CostMonitoringPageProps) {
  const serviceBreakdown = services.map((service) => ({
    name: service.name,
    type: service.id,
    estimatedCost: 0.0, // Default to $0.00
  }));
        ...
        <Card className="mb-6 border-orange-200">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-4xl font-medium bg-gradient-to-r from-orange-600 to-orange-800 bg-clip-text text-transparent">
                ${totalCost.toFixed(2)}
              </p>
              <p className="text-sm text-muted-foreground mt-1">
                {serviceBreakdown.length} service
                {serviceBreakdown.length !== 1 ? "s" : ""}
              </p>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="space-y-4">
              {serviceBreakdown.length > 0 ? (
                serviceBreakdown.map((service, index) => (
                  <div
                    key={index}
                    className="flex items-center justify-between p-4 border rounded-lg hover:border-orange-200 hover:bg-orange-50/30 transition-colors"
                  >
                    <div>
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">
                    Deploy some AWS services to see cost breakdown
                  </p>
                </div>
              )}
            </div>
