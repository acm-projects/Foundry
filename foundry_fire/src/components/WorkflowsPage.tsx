...
export function WorkflowsPage({ workflows, onCreateNew, onOpenWorkflow }: WorkflowsPageProps) {
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {displayWorkflows.map((workflow) => (
            <Card key={workflow.id} className="hover:shadow-md transition-shadow cursor-pointer group hover:border-orange-200">
              <CardHeader>
              ...
              <CardContent>
                <div className="space-y-4">
                  {/* Services */}
                  <div>
                    <p className="text-sm text-muted-foreground mb-2">Services ({workflow.services.length})</p>
                    <div className="flex flex-wrap gap-1">
                      {workflow.services.slice(0, 4).map((service, index) => (
                        <Badge key={index} variant="outline" className="text-xs hover:border-orange-300 hover:bg-orange-50 transition-colors">
                          {getServiceIcon(service.id)} {service.id.toUpperCase()}
                        </Badge>
                      ))}
